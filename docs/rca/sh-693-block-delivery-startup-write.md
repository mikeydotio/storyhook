...
- **Status**: Fixed in `3ff9d9c00` (worker), `85af3bd32` (harness diagnosis), `feba4b3f0` (golden corpus, the second casualty) and `fa0e23e60` (spawn inventory, the third); story SH-693.
...
The third casualty surfaced one gate later, once the rust-suite leg was green: the contracts leg failed `tests/spawn_inventory.rs`, because SH-690's helper spawn in `src/daemon/block_delivery.rs` had never been classified. It is `Waited` — the shared captured-run helper gives the child files rather than pipes, a process group and a bounded wait — and `fa0e23e60` says so. The pattern across all three is the same: a merge under a terminated gate leaves every contract that gate would have run unexercised, and each surfaces only when the next branch pays for a full run.
