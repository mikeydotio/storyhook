...
| 2 | order by priority then queue age | effective priority (the stored level raised to its blocker floor, SH-788) → `verifying_since` → project slug → story id (`sort_candidates`); missing entry timestamps follow known timestamps within equal priority | done, SH-651 (D-F) |
