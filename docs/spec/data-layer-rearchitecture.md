...
| **W1** | `feat(store): event-sourced Store trait with SQLite engine, migrations, and rebuild-diff` | Pure new code, `src/store/`, nothing wired. Trait leaks no rusqlite types. WAL, `BEGIN IMMEDIATE`. Schema enforces relation symmetry and originally enforced single-parent as constraints (SH-446 later made multiple parents intentional). Unknown-event tolerance + `schema_version` gate (kills SH-54 class). Versioned migrations w/ verified backup. `rebuild_read_model()`/`diff_read_model()` oracle. Transactional ID allocation. | L | 12% |
