...

{
  ...
  printf -- '- **`SyncConflict` (exit 8) is dead code.** It is constructed nowhere in\n'
  printf -- '  `src/`; only `src/web.rs` maps it to HTTP 409. `%s` covers its\n' "$CONTRACT_SRC"
  printf -- '  exit code at the enum level and lists it `UNREACHABLE`. See\n'
  printf -- '  `docs/rearch/STATE.md`, W0.3 finding #3 — it needs a story, or a deletion.\n'
  printf -- '- **`GithubAuth` / `GithubApi` are feature-gated** behind the default-on\n'
  printf -- '  `github-sync` feature. `--no-default-features` compiles them out, and the\n'
  printf -- '  contract test skips their rows accordingly; a capture taken without the\n'
  printf -- '  feature would show them as unreachable.\n'
