...
test("hiding a column via the Columns dropdown removes its cards without changing the filter count", async ({
  ...
  await expect(page.locator("#filter-count")).toHaveText("2 / 2");
  // The accessible NAME, never textContent. Since SH-620 this button holds a
  // decorative `aria-hidden` emoji caret beside its label, and textContent
  // still carries it -- `toHaveText` here read "Columns (1)\u{1F53D}". Name
  // computation excludes aria-hidden subtrees by specification, so this asserts
  // the label a user is actually told and nothing decorative. Any assertion
  // about a control's own words wants the name; only a spec whose SUBJECT is
  // the glyph asserts text (SH-622).
  await expect(page.locator("#fdd-columns .fdd-btn")).toHaveAccessibleName("Columns (1)");
  ...
  await expect(page.locator("#fdd-columns .fdd-btn")).toHaveAccessibleName("Columns");
});
