...
// Each case owns Alpha's origin only for its duration; other specs exercise
// dispatch without a remote. These are fixture identities, never live endpoints.
for (const host of ["github.com", "github.pie.apple.com"]) {
  test.describe(`origin ${host}`, () => {
    const checkout = requiredEnv("DASHBOARD_ALPHA_CHECKOUT");
    const config = join(checkout, ".git", "config");
    let originalConfig: string;
    test.beforeEach(() => {
      originalConfig = readFileSync(config, "utf8");
      expect(originalConfig).not.toContain('[remote "origin"]');
      writeFileSync(config, `${originalConfig}\n[remote "origin"]\n\turl = https://${host}/acme/widgets.git\n`);
    });
    test.afterEach(() => {
      writeFileSync(config, originalConfig);
    });

    test(`an open PR chip uses ${host} in a new tab without opening the drawer`, async ({
      page,
      request,
    }) => {
      ...
      const prUrl = `https://${host}/acme/widgets/pull/586`;
