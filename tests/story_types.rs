...
fn mcp_create_story_with_type() {
    ...
    let input = r#"{"jsonrpc":"2.0","id":1,"method":"tools/call","params":{"name":"storyhook_create_story","arguments":{"title":"Login crash","story_type":"bug"}}}"#;
    let output = mcp_request(dir.path(), input);
    let response: serde_json::Value = serde_json::from_str(output.trim()).unwrap();
    let content = response["result"]["content"][0]["text"].as_str().unwrap();
    // MCP returns JSON (json: true), so the story_type field is in the JSON structure
    assert!(content.contains("\"story_type\": \"bug\""));
}

#[test]
fn mcp_update_story_type() {
    let dir = tempdir().unwrap();
    Command::cargo_bin("story")
        .unwrap()
        .current_dir(dir.path())
    ...
    let input = r#"{"jsonrpc":"2.0","id":1,"method":"tools/call","params":{"name":"storyhook_update_story","arguments":{"id":"SH-1","story_type":"epic"}}}"#;
    let output = mcp_request(dir.path(), input);
    let response: serde_json::Value = serde_json::from_str(output.trim()).unwrap();
    let content = response["result"]["content"][0]["text"].as_str().unwrap();
    // SetFields returns a message like "updated SH-1: type -> epic"
    assert!(content.contains("type -> epic"));
}

#[test]
fn mcp_list_stories_with_type_filter() {
    let dir = tempdir().unwrap();
    Command::cargo_bin("story")
        .unwrap()
        .current_dir(dir.path())
    ...
    let input = r#"{"jsonrpc":"2.0","id":1,"method":"tools/call","params":{"name":"storyhook_list_stories","arguments":{"story_type":"bug"}}}"#;
    let output = mcp_request(dir.path(), input);
    let response: serde_json::Value = serde_json::from_str(output.trim()).unwrap();
    let content = response["result"]["content"][0]["text"].as_str().unwrap();
