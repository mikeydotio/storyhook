...
const ALLOWED: [(&str, &str); 8] = [
    (
        "src/service/project_recovery/authority.rs",
        "recovery reads historical creation and state transitions to retain authority; this pattern constructs no events",
    ),
    (
        "src/domain.rs",
