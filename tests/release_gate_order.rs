...
"#,
        ...
        storyhook_test_support::install_git_endpoint(
            ...
            &[(
                "https://github.pie.apple.com/acme/storyhook.git",
...
[ "$GH_REPO" = github.pie.apple.com/acme/storyhook ] || exit 93
case "$1" in
  api) [[ " $* " = *' --hostname github.pie.apple.com '* ]] || exit 94 ;;
  release) [[ " $* " = *' --repo github.pie.apple.com/acme/storyhook '* ]] || exit 95 ;;
  *) exit 96 ;;
esac
case "$1 $2" in
...
"#,
    ...
    assert!(
        String::from_utf8_lossy(&output.stdout)
            .contains("https://github.pie.apple.com/acme/storyhook/releases/tag/v9.9.9")
    );
    let write = fs::read_to_string(fixture.scratch.path().join("published")).unwrap();
    assert!(
        write.contains(
            "release edit v9.9.9 --draft=false --repo github.pie.apple.com/acme/storyhook"
        )
    );
}
