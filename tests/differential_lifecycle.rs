...
//!
//! The rows here are the story lifecycle and relations. The harness they run
//! on — including the single deliberate normalization, timestamps — lives in
//! [`differential_support`]; the project, configuration and system families
//! have their own rows in `differential_config.rs`.
...
use differential_support::{Differential, story_id};
...
fn a_rejected_initial_state_burns_a_story_number_in_the_legacy_leg_only() {
    ...
    let legacy = differential.legacy_only(new_story("after the rejection"));
    let new = differential.store_only(new_story("after the rejection"));
