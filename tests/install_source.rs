...
fn bootstrap_requires_source_and_persists_enterprise_authority() {
    ...
    let output = run(&["--source", "github.example.com/acme/storyhook"]);
    assert!(
    ...
    assert!(
        calls
            .lines()
            .all(|l| l.starts_with("github.example.com|github.example.com/acme/storyhook|"))
    );
    assert!(
        calls
            .lines()
            .all(|l| l.contains("--repo github.example.com/acme/storyhook"))
    );
