...
fn settle_claim_resume(project: &Project<'_>, id: &str) {
    ...
    let number = project.story_no(&store, id);
    let resume = store
    ...
    let settled = loop {
        let record = store
            ...
            .unwrap()
            .into_iter()
            .find(|delivery| delivery.id == resume.id)
            .expect("the original resume intent must remain in history");
        assert_eq!(record.story, number, "{record:?}");
        assert_eq!(record.action, BlockAction::Resume, "{record:?}");
        if !matches!(
            record.status,
    ...
    // Observe release; never force-unlock another owner's inherited descriptor.
    let path = project
        .path()
        .join(format!(".git/storyhook/workspace-locks/{id}.lock"));
    let observer = std::fs::OpenOptions::new()
        ...
        .unwrap();
    loop {
        match observer.try_lock_exclusive() {
            Ok(()) => break,
            ...
            Err(error) => panic!("observing workspace release at {}: {error}", path.display()),
        }
    }
    // Only this test's observation lock is released; it owns no effect children.
...
fn reset_always_returns_to_todo_and_can_be_repeated_without_resources() {
    ...
    settle_claim_resume(&project, &id);
    for _ in 0..2 {
