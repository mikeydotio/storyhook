...
fn public_and_enterprise_origins_keep_their_destination() {
    for host in ["github.com", "github.pie.apple.com"] {
        for origin in [
...
fn https_port_is_preserved_and_ssh_port_is_not_guessed() {
    let root = checkout("https://github.pie.apple.com:8443/acme/widgets.git");
    assert_eq!(
        Repository::resolve(root.path()).unwrap().identity().host,
        "github.pie.apple.com:8443"
    );
    git(
        root.path(),
        &[
            ...
            "set-url",
            "origin",
            "ssh://git@github.pie.apple.com:2222/acme/widgets",
        ],
    );
    assert!(
...
fn changed_origin_is_read_again_instead_of_registered_or_cached_identity() {
    ...
    git(
        ...
        &[
            ...
            "set-url",
            "origin",
            "git@github.pie.apple.com:new/moved.git",
        ],
    );
    assert_eq!(
        Repository::resolve(root.path()).unwrap().qualified(),
        "github.pie.apple.com/new/moved"
    );
}
...
fn helper_overrides_ambient_routing_and_passes_explicit_enterprise_destination() {
    ...
    assert_eq!(
        lines,
        "github.pie.apple.com\ngithub.pie.apple.com/acme/widgets\n1\nunset\npr\nview\n7\n--json\nstate\n--repo\ngithub.pie.apple.com/acme/widgets\n"
    );
}

#[test]
fn api_calls_use_explicit_host_and_repository_paths() {
    let root = checkout("https://github.pie.apple.com/acme/widgets.git");
    let output = helper(
        root.path(),
        &[
            "exec",
...
fn failed_gh_reports_context_without_credentials() {
    ...
    assert!(error.contains("github.pie.apple.com/acme/widgets"));
    assert!(error.contains("gh auth login --hostname github.pie.apple.com"));
...
fn resolve_helper_is_local_and_returns_structured_origin() {
    ...
    assert_eq!(value["identity"]["host"], "github.pie.apple.com");
    assert!(
...
fn git_transport_uses_host_scoped_gh_credentials_and_explicit_https_origin() {
    for origin in [
        ...
        for operation in ["fetch", "push", "ls-remote"] {
            ...
            assert!(
                output.contains(
                    "credential.https://github.pie.apple.com.helper=!gh auth git-credential"
                ),
                "{output}"
            );
            let destination = if operation == "push" {
                assert!(
                    output.contains(&format!(
                        "url.https://github.pie.apple.com/acme/widgets.git.insteadOf={origin}\n"
                    )),
                    "{output}"
                );
                "origin"
...
fn generic_enterprise_observations_use_the_https_credential_boundary() {
    ...
    let output = String::from_utf8(output.stdout).unwrap();
    assert!(
        output.contains("credential.https://github.pie.apple.com.helper=!gh auth git-credential")
    );
    assert!(
...
fn observation_mirrors_require_matching_current_source_authority() {
    let root = checkout("git@github.pie.apple.com:acme/widgets.git");
    let source = checkout("https://github.pie.apple.com/acme/widgets.git");
    recording_transport(root.path());
    let call = || {
        helper(
            root.path(),
...
fn a_competing_exact_rewrite_cannot_restore_ssh_transport() {
    let origin = "git@github.pie.apple.com:acme/widgets.git";
    let root = checkout(origin);
    git(
        root.path(),
        &[
            "config",
            "url.ssh://git@github.pie.apple.com/acme/widgets.git.insteadOf",
            origin,
        ],
    );
    recording_transport(root.path());
