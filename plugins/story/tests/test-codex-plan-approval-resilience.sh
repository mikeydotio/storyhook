...
run_watcher() {
  ...
  PATH="$TMUX_FIXTURE:$PATH" \
    bash "$HOOK" --approve-codex-plan "$pane" "$expected_pid" "$limit"
}
...
# Independent watcher processes have no shared retry or Return budget.
reset_case
run_watcher normal %4242 777 & first=$!
run_watcher normal %4343 777 & second=$!
wait "$first"
wait "$second"
assert_eq "$(value %4242 send)" 1 "first concurrent pane has its own Return budget"
assert_eq "$(value %4343 send)" 1 "second concurrent pane has its own Return budget"
assert_eq "$(send_log %4242)" 'send-keys -t %4242 Enter' "first concurrent watcher targets only its pane"
assert_eq "$(send_log %4343)" 'send-keys -t %4343 Enter' "second concurrent watcher targets only its pane"
...
if [ "$failures" -ne 0 ]; then
  printf 'FAIL: %s Codex plan approval resilience assertion(s) failed.\n' "$failures" >&2
  exit 1
fi
