...
pub fn export_project(root: &Path) -> Result<ProjectExport, AppError> {
    ...
    Ok(ProjectExport {
        ...
        remotes: Vec::new(),
        // Tombstones only (SH-408) — the rollback leg never writes either any
        // more, so re-reading a tree this crate wrote always finds neither.
        // See `ProjectExport::github_sync`'s own doc comment.
        github_sync: None,
...
pub fn import_project(
    ...
    // `export.github_sync`/`github_bases` are tombstones (SH-408) — the store
    // no longer holds either, so a current export never carries one and there
    // is nothing left to write here. See `ProjectExport::github_sync`'s own
    // doc comment.
...
mod tests {
    ...
    // --- github-sync tombstones, retired with the rollback leg (SH-408) ---

    #[test]
    fn export_project_never_carries_a_github_sync_blob_or_bases() {
        let dir = setup_project();
        let export = export_project(dir.path()).unwrap();
        assert_eq!(export.github_sync, None);
        assert!(export.github_bases.is_empty());
    }

    #[test]
    fn import_project_does_not_recreate_the_legacy_github_sync_layout() {
        ...
        let document = ProjectExport {
            ...
            // never does. Either way the rollback leg no longer writes them.
            github_sync: Some(blob),
        ...
        assert!(!dir.path().join(".storyhook/github-sync/bases").exists());
    }
}
