...
pub(super) fn put_verification_incident(
    conn: &Connection,
    ...
    ), "writing the verification incident")?;
    Ok(())
}

pub(super) fn clear_verification_incident(
    conn: &Connection,
    incident_id: &str,
...
pub(super) fn put_legacy_story_reset(
    conn: &Connection,
    project: ProjectId,
    story: StoryNo,
    ...
    if let Some(value) = reservation {
        sql(conn.execute("INSERT INTO story_reset_reservations(project_id, story_no, reservation) VALUES (?1, ?2, ?3) ON CONFLICT(project_id, story_no) DO UPDATE SET reservation = excluded.reservation", params![project.get(), story.get(), value]), "writing reset reservation")?;
    } else {
        sql(
            conn.execute(
                "DELETE FROM story_reset_reservations WHERE project_id = ?1 AND story_no = ?2",
                params![project.get(), story.get()],
            ),
