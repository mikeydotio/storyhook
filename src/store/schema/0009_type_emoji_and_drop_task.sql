-- storyhook store — schema version 9: a type carries an emoji, and `task` is
-- retired (SH-157).
--
-- ---------------------------------------------------------------------------
-- The new column
...
-- (`WHERE emoji IS NULL`, so a project that already renamed or redescribed
-- one of them keeps whatever it holds — this only fills a gap, never
-- overwrites a choice). A project's custom types, and any project that
-- renamed a default before this ran, are left with `emoji IS NULL`; the
...
--
-- What is NOT done: a bare `UPDATE stories SET story_type = 'story' WHERE
-- story_type = 'task'`. The event log is this store's source of truth, and
-- `story doctor` / `store::rebuild::diff` compare every row against the fold
-- of its own events — a row edited without a matching event is permanent,
-- self-inflicted drift, indistinguishable from corruption the next time
-- either of those runs. Migration 4 solved its equivalent problem by changing
-- the *fold* instead; that tool does not fit here, because a fold change
-- would make the slug `task` unrepresentable forever, which is the opposite
-- of "removed, not reserved".
--
-- So each `task`-typed story gets a real, appended `StoryTypeSet` event
-- (legal: the append-only triggers on `events` block UPDATE and DELETE, not
-- INSERT), and the row is then brought forward to agree with it — the same
-- shape `put_story` uses after any other event append, just written as SQL
-- because a migration has no `fold_story` to call.
--
...
-- it's invoked; three separate calls across three statements could
-- theoretically straddle a second boundary and disagree, which would make
-- `events.at` and `stories.updated_at` diverge for no reason a reader could
-- see. Fixing it once removes that possibility rather than hoping the
-- migration runs fast enough to avoid it. The temp table lives in SQLite's
-- per-connection temp database, not in `sqlite_master`, so it is invisible to
...
UPDATE project_types
   ...
   END
 WHERE emoji IS NULL
   AND slug IN ('story', 'epic', 'bug', 'chore');

CREATE TEMP TABLE sh157_now AS
SELECT strftime('%Y-%m-%dT%H:%M:%SZ', 'now') AS ts;

-- One `StoryTypeSet` event per `task`-typed story, appended after that
-- story's current head. `global_seq` is allocated from the project's own
-- counter, offset by this story's rank among its project's task-typed
-- stories (`ROW_NUMBER() ... - 1`), so two stories in the same project land
-- on two consecutive, non-colliding values — exactly what `append_events`
-- would hand out one call at a time.
...
UPDATE projects
   SET next_global_seq = next_global_seq + (
       SELECT COUNT(*) FROM stories s
        WHERE s.project_id = projects.id AND s.story_type = 'task'
   )
 WHERE EXISTS (
...
UPDATE stories
   SET story_type = 'story',
       ...
       snapshot = json_set(
           json_set(snapshot, '$.story_type', 'story'),
           '$.updated_at', (SELECT ts FROM sh157_now)
       )
 WHERE story_type = 'task';
